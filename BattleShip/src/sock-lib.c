...
int conectar (char argv[2][20])
{
	...
	if ((he = gethostbyname ((const char *) argv[0])) == NULL)
	{
		return ERROR;
	}
...
int Open_conection (struct sockaddr_in * my_addr, char* ip, int port){
...
/* Asignamos valores a la estructura my_addr */
	...
	my_addr->sin_addr.s_addr = (!strcmp(ip,"localhost"))? INADDR_ANY:inet_addr(ip);//;i"192.168.0.74"	/* automaticamente usa la IP local */
